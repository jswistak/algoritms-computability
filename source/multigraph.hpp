...
bool edgeExists(int u, int v, const std::vector<std::vector<int>>& G);
int findPivot(const std::set<int>& P, const std::set<int>& X, const std::vector<std::vector<int>>& adjMatrix);
...
int get_random_element_from_set(std::set<int> s);
